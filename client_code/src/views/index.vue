<template>
	<div class="home">
		<div class="index_top">
			<div class="index_top_title">基于SpringBoot高校毕业生招聘就业管理系统</div>
...
<script setup>
	...
	const getSession = () =>{
		context?.$http({
			...
			if(context?.$toolUtil.storageGet('frontSessionTable') == 'daxueshengyonghu'){
				context?.$toolUtil.storageSet("frontName", res.data.data.zhanghao)
			}
			if(context?.$toolUtil.storageGet('frontSessionTable') == 'daxueshengyonghu'){
				context?.$toolUtil.storageSet('headportrait',res.data.data.touxiang)
			}
			if(context?.$toolUtil.storageGet('frontSessionTable') == 'qiyeyonghu'){
