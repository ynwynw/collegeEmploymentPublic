		...
		<el-form :inline="true" :model="searchQuery" class="list_search">
			<div class="search_view">
				...
				<div class="search_box">
					<el-input class="search_inp" v-model="searchQuery.zhiweimingcheng" placeholder="职位名称"
						clearable>
					</el-input>
				</div>
			</div>
			<div class="search_btn_view">
				...
				<el-button class="add_btn" type="success" v-if="btnAuth('xiaozhaogangweifabu','新增')" @click="addClick">新增</el-button>
			</div>
		</el-form>
				...
				<div class="data_view">
					<div class="data_item animation_box" v-for="(item,index) in list" :key="index" @click.stop="detailClick(item.id)" >
						...
						<div class="data_content">
							<div class="data_title">{{item.zhiweimingcheng}}</div>
							<div class="data_title">{{item.gongsiyaoqiuxueli}}</div>
...
<script setup>
	...
	const addClick = () => {
		router.push('/index/xiaozhaogangweifabuAdd')
	}
	//判断是否从个人中心跳转
	...
	const detailClick = (id) => {
		router.push('xiaozhaogangweifabuDetail?id=' + id + (centerType.value?'&&centerType=1':''))
	}
	//下载文件
	const download = (file) =>{
		if(!file){
