	...
	<div class="app-contain" :style='{"minHeight":"100vh","padding":"0 0 40px","margin":"0 auto","overflow":"hidden","borderRadius":"4px","background":"#eee","width":"86%","position":"relative"}'>
		...
		<el-form :inline="true" :model="searchQuery" class="list_search">
			...
			<div class="search_btn_view">
				...
				<el-button class="add_btn" type="success" v-if="btnAuth('zaixianzixun','新增')" @click="addClick">新增</el-button>
			</div>
		</el-form>
				...
				<div class="data_view">
					<div class="data_item animation_box" v-for="(item,index) in list" :key="index" @click.stop="detailClick(item.id)" >
						<div class="data_content">
							...
							<div class="data_title">{{item.xinzi}}</div>
							<div class="data_title">{{item.qiyemingcheng}}</div>
							<div class="data_title">{{item.zixunshijian}}</div>
...
<script setup>
	...
	const addClick = () => {
		router.push('/index/zaixianzixunAdd')
	}
	//判断是否从个人中心跳转
	...
	const detailClick = (id) => {
		router.push('zaixianzixunDetail?id=' + id + (centerType.value?'&&centerType=1':''))
	}
	//下载文件
	const download = (file) =>{
		if(!file){
